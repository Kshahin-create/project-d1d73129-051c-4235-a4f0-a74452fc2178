...
const endpoints: Endpoint[] = [
  ...
  },
  {
    ...
    path: "/units",
    title: "إضافة وحدة",
    desc: "إنشاء وحدة جديدة في مبنى. يتطلب صلاحية `write`.",
    scope: "write",
    bodyParams: [
      { name: "building_number", type: "integer", required: true, desc: "رقم المبنى" },
      { name: "unit_number", type: "integer", required: true, desc: "رقم الوحدة" },
      { name: "unit_type", type: "string", desc: "نوع الوحدة (ركنية/داخلية)" },
      { name: "area", type: "number", desc: "المساحة بالمتر المربع" },
      { name: "activity", type: "string", desc: "النشاط" },
      { name: "price", type: "number", desc: "السعر السنوي" },
      { name: "status", type: "string", desc: "available | rented | reserved" },
    ],
    body: { building_number: 1, unit_number: 105, unit_type: "ركنية", area: 50, activity: "قطع غيار", price: 60000, status: "available" },
    response: { data: { id: "uuid", building_number: 1, unit_number: 105 } },
  },
  {
    id: "update-unit",
    method: "PATCH",
    path: "/units/:id",
    title: "تعديل وحدة",
    desc: "تحديث جزئي لبيانات الوحدة (السعر، الحالة، النشاط...). يتطلب صلاحية `write`.",
    scope: "write",
    pathParams: [{ name: "id", type: "uuid", required: true, desc: "معرف الوحدة" }],
    body: { price: 65000, status: "available" },
    response: { data: { id: "uuid", price: 65000, status: "available" } },
  },
  {
    id: "delete-unit",
    method: "DELETE",
    path: "/units/:id",
    title: "حذف وحدة",
    desc: "حذف الوحدة نهائياً. يتطلب صلاحية `admin`.",
    scope: "admin",
    pathParams: [{ name: "id", type: "uuid", required: true, desc: "معرف الوحدة" }],
    response: { success: true },
  },
  {
    ...
    desc: "كل الحجوزات مع وحداتها. يتطلب صلاحية `write`.",
    scope: "write",
    queryParams: [
      { name: "status", type: "string", desc: "pending | confirmed | cancelled", example: "pending" },
      { name: "user_id", type: "uuid", desc: "تصفية بحسب المستخدم" },
    ],
    response: {
      data: [
        {
          ...
          booking_units: [{ building_number: 1, unit_number: 101, area: 50, price: 60000 }],
        },
      ],
      count: 25,
    },
  },
  {
    ...
    path: "/bookings/:id",
    title: "تفاصيل حجز",
    desc: "تفاصيل حجز محدد بكل وحداته.",
    scope: "write",
    pathParams: [{ name: "id", type: "uuid", required: true, desc: "معرف الحجز" }],
    response: { data: { id: "uuid", status: "pending", booking_units: [] } },
  },
  {
    id: "update-booking",
    method: "PATCH",
    path: "/bookings/:id",
    title: "تعديل حجز",
    desc: "تحديث حالة الحجز أو الملاحظات أو علامة إرسال الواتساب.",
    scope: "write",
    pathParams: [{ name: "id", type: "uuid", required: true, desc: "معرف الحجز" }],
    bodyParams: [
      { name: "status", type: "string", desc: "pending | confirmed | cancelled" },
      { name: "notes", type: "string", desc: "ملاحظات" },
      { name: "whatsapp_sent", type: "boolean", desc: "علامة إرسال الواتساب" },
    ],
    body: { status: "confirmed" },
    response: { data: { id: "uuid", status: "confirmed" } },
  },
  {
    id: "list-customers",
    method: "GET",
    path: "/customers",
    title: "قائمة العملاء",
    desc: "بيانات الملفات الشخصية للعملاء (الاسم، الجوال، الإيميل، النشاط، الصورة).",
    scope: "write",
    queryParams: [
      { name: "search", type: "string", desc: "بحث في الاسم أو الجوال أو الإيميل", example: "أحمد" },
    ],
    response: {
      data: [
        {
  ...
  },
  {
    ...
    title: "تفاصيل عميل",
    desc: "بيانات عميل محدد بمعرف المستخدم.",
    scope: "write",
    pathParams: [{ name: "user_id", type: "uuid", required: true, desc: "معرف المستخدم" }],
    response: { data: { user_id: "uuid", full_name: "أحمد محمد" } },
  },
  {
    id: "list-users",
    method: "GET",
    ...
    desc: "كل المستخدمين المسجلين مع أدوارهم. يتطلب صلاحية `admin`.",
    scope: "admin",
    response: {
      data: [
        {
  ...
  },
  {
    ...
    path: "/audit-log",
    title: "سجل التدقيق",
    desc: "سجل كل التعديلات (INSERT/UPDATE/DELETE) على جداول النظام مع الفاعل والقيم القديمة/الجديدة. يتطلب صلاحية `admin`.",
    scope: "admin",
    queryParams: [
      { name: "table", type: "string", desc: "اسم الجدول للتصفية", example: "bookings" },
      { name: "action", type: "string", desc: "INSERT | UPDATE | DELETE", example: "UPDATE" },
    ],
    response: {
      data: [
        {
          ...
          before_data: { status: "available", price: 60000 },
          after_data: { status: "rented", price: 60000 },
